...
const CategoryDropdown = (props: Props) => {
  ...
  return (
    <div className="my-6">
      <button type="button" onClick={toggleOpen} className="px-3 py-3 bg-yellow-500 rounded-md border">
        {currSelection.name}
      </button>
      {open ? (
        <div className="absolute mt-2 bg-white rounded-md border neo-shadow">
          {props.list.map((item) => (
            <div className="px-3 py-2 hover:bg-gray-100 cursor-pointer" key={item.id} onClick={() => selectOption(item.id)}>
              {item.name}
            </div>
          ))}
        </div>
